...
using Logger = util::Logger<util::MT_logger_writer>;

template<typename... Args>
inline void log_info(const std::source_location &loc, std::string_view fmt, Args &&...args) noexcept {
  auto logger = util::make_logger(util::MT_logger_writer(std::cerr), util::Log_level::Info);
  logger.info(loc, fmt, std::forward<Args>(args)...);
}

template<typename... Args>
inline void log_debug(const std::source_location &loc, std::string_view fmt, Args &&...args) noexcept {
  auto logger = util::make_logger(util::MT_logger_writer(std::cerr), util::Log_level::Debug);
  logger.debug(loc, fmt, std::forward<Args>(args)...);
}

template<typename... Args>
inline void log_trace(const std::source_location &loc, std::string_view fmt, Args &&...args) noexcept {
  auto logger = util::make_logger(util::MT_logger_writer(std::cerr), util::Log_level::Trace);
  logger.trace(loc, fmt, std::forward<Args>(args)...);
}

template<typename... Args>
inline void log_warn(const std::source_location &loc, std::string_view fmt, Args &&...args) noexcept {
  auto logger = util::make_logger(util::MT_logger_writer(std::cerr), util::Log_level::Warn);
  logger.warn(loc, fmt, std::forward<Args>(args)...);
}

template<typename... Args>
inline void log_error(const std::source_location &loc, std::string_view fmt, Args &&...args) noexcept {
  auto logger = util::make_logger(util::MT_logger_writer(std::cerr), util::Log_level::Err);
  logger.err(loc, fmt, std::forward<Args>(args)...);
}

template<typename... Args>
inline void log_fatal(const std::source_location &loc, std::string_view fmt, Args &&...args) noexcept {
  auto logger = util::make_logger(util::MT_logger_writer(std::cerr), util::Log_level::Fatal);
  logger.fatal(loc, fmt, std::forward<Args>(args)...);
}

/* Macros to capture source_location at call site */
#define trace(logger, fmt, ...) \
  (logger).trace(std::source_location::current(), fmt, ##__VA_ARGS__)

#define debug(logger, fmt, ...) \
  (logger).debug(std::source_location::current(), fmt, ##__VA_ARGS__)

#define info(logger, fmt, ...) \
  (logger).info(std::source_location::current(), fmt, ##__VA_ARGS__)

#define warn(logger, fmt, ...) \
  (logger).warn(std::source_location::current(), fmt, ##__VA_ARGS__)

#define log_error(logger, fmt, ...) \
  (logger).err(std::source_location::current(), fmt, ##__VA_ARGS__)

#define fatal(logger, fmt, ...) \
  (logger).fatal(std::source_location::current(), fmt, ##__VA_ARGS__)

#define log_inf(fmt, ...) log_info(std::source_location::current(), fmt, ##__VA_ARGS__)
#define log_dbg(fmt, ...) log_debug(std::source_location::current(), fmt, ##__VA_ARGS__)
#define log_trc(fmt, ...) log_trace(std::source_location::current(), fmt, ##__VA_ARGS__)
#define log_wrn(fmt, ...) log_warn(std::source_location::current(), fmt, ##__VA_ARGS__)
#define log_err(fmt, ...) log_error(std::source_location::current(), fmt, ##__VA_ARGS__)
#define log_ftl(fmt, ...) log_fatal(std::source_location::current(), fmt, ##__VA_ARGS__)